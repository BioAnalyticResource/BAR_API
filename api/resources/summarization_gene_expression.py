    ...
    def post(self):
        ...
        if(request.method == "POST"):
            ...
            if(SummarizationGeneExpressionUtils.decrement_uses(key)):
                ...
                path = os.path.join(app.config["SUMMARIZATION_FILES_PATH"], "rpkm.wdl")
                files = {'workflowSource': ('rpkm.wdl', open(path, 'rb')), 'workflowInputs': ('rpkm_inputs.json', inputs)}
                requests.post(app.config["CROMWELL_URL"] + '/api/workflows/v1', files=files)
    ...
    def post(self):
        ...
        if(request.method == "POST"):
            ...
            if file:
                ...
                key = request.headers.get("X-Api-Key")
                file.save(os.path.join(app.config["DATA_FOLDER"] + "/" + key + "/", filename))
                if(SummarizationGeneExpressionUtils.decrement_uses(key)):
                    inputs = """
                            {
                            "csvUpload.insertDataScript": "./insertData.py",
                            "csvUpload.id": """ + key + """,
                            "csvUpload.csv": """ + os.path.join(app.config["DATA_FOLDER"], key, filename) + """,
                            }
                            """
                    path = os.path.join(app.config["SUMMARIZATION_FILES_PATH"], "csvUpload.wdl")
        ...
        if (request.method == "POST"):
            ...
            if(api_key is None):
                ...
                if 'file' in request.files:
                    ...
                    filename = os.path.join(app.config["DATA_FOLDER"], api_key, dt_string + extension)
                    file.save(filename)
                    return BARUtils.success_exit(True)
                else:
                    return BARUtils.error_exit("No file attached"), 400
...
class SummarizationGeneExpressionGetFileList(Resource):
    def post(self):
        ...
        if (request.method == "POST"):
            api_key = request.headers.get('x-api-key')
            files = []
            for file in os.walk(app.config["DATA_FOLDER"] + api_key):
                files.append(file[2])
...
class SummarizationGeneExpressionGetFile(Resource):
    ...
    def get(self, file_id):
        ...
        if (request.method == "GET"):
            api_key = request.headers.get('x-api-key')
            filename = os.path.join(app.config["DATA_FOLDER"], api_key, file_id)
