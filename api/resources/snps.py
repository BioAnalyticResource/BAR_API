...
parser = snps.parser()
parser.add_argument('snps', type=str, action='append', required=True, help='SNP locations, format: OriLocMut i.e. V25L', default=["V25L", "E26A"])
...
class Pymol(Resource):
    ...
    def get(self, model):
        ...
        if BARUtils.is_arabidopsis_gene_valid(model):
            ...
            gene_pdb_path = (
                    poplar_pdb_path + BARUtils.format_poplar(model) + ".pdb"
            )
        ...
        for each in snps:
            if re.match('^[a-zA-Z][1-9][0-9]*[a-zA-Z]$', each) is None:
                ...
                return BARUtils.error_exit("Invalid SNP string for protein letters"), 400
            else:
        ...
        if list_len > 0:
            return BARUtils.error_exit("Conflict SNPs input at loci: %s"
                                       % [int(x) for x in conflict_snps_loc]), 400
