...
class ProteinReference(db.Model):
    ...
    gene_name: db.Mapped[Optional[str]] = db.mapped_column(db.String(45), primary_key=False)
...
class SamplesLookup(db.Model):
    __bind_key__ = "soybean_nssnp"
    __tablename__ = "sample_lookup"
    sample_id: db.Mapped[Optional[str]] = db.mapped_column(db.String(45), primary_key=True)
    dataset: db.Mapped[Optional[str]] = db.mapped_column(db.String(45), primary_key=False)
    dataset_sample: db.Mapped[Optional[str]] = db.mapped_column(db.String(45), primary_key=False)
